...
synths = Build(synth_top, synth_top, synth_top, FilesetBuild(match = ['$SRCDIR/module/mac10g_top/hdl/*.vhd']))
synths.srcs_setup['args'].target = ['mac10g_receive.synth', 'mac10g_transmit.synth', 'mac10g_axi_lite.synth', 'all']
...
hdl_fileset = FilesetCombinedCopy(synths, FileBuild('$BUILDDIR/ip/mac10g/hdl'))
ipprj = VivadoIpProjectBuild(name     = 'ippack',
                           prjdir   = FileBuild(os.path.join('$BUILDDIR', 'mac10g_pack', 'ippack')),
                           ipdir    = FileBuild('$BUILDDIR/ip/mac10g'),
                           sources  = {'sources_1': hdl_fileset},
                           ipconfig = {'vendor' : 'bogdanvuk.org', 'name': 'hlsmac10g', 'library': 'ip', 'display_name': '{HLS 10G MAC}'}, 
